<?php
	...
	//-------------------------------------------------------------------------------------------------------
	// Functions for anonymizing personal information
	//-------------------------------------------------------------------------------------------------------
	function PinfoEncrypter($str, $key)
	{
		...
		for($n=0; $n<strlen($str); $n++)
		{
			...
			{
				if($inChar  === $baseStr[$i])  $inNum  = $i;
				if($keyChar === $baseStr[$i])  $keyNum = $i;
			}
			...
			{
				$encodeNum = ($inNum + $keyNum) % strlen($baseStr);
				$ret .= $baseStr[$encodeNum];
			}
		}
	...
	function PinfoDecrypter($str, $key)
	{
		$baseStr = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789abcdefghijklmnopqrstuvwxyz';
		$ret = "";
		...
		for($n=0; $n<strlen($str); $n++)
		{
			...
			{
				$encodeNum = ($inNum - $keyNum + strlen($baseStr)) % strlen($baseStr);
				$ret .= $baseStr[$encodeNum];
			}
		}
	...
	function ScramblePatientName()
	{
		return 'XXXXX XXXXX';
	}

	function ScrambleBirthDate()
	{
		return 'YYYY-MM-DD';
	}
	//-------------------------------------------------------------------------------------------------------	
