<?php
	...
	{	
		...
		if($oldTodayDisp != $newTodayDisp || $oldDarkroomFlg != $newDarkroomFlg || $oldLatestResults != $newLatestResults)
		{
			$stmt = $pdo->prepare("UPDATE users SET today_disp=?, darkroom_flg=?, latest_results=? WHERE user_id=?");
			$stmt->execute(array($newTodayDisp, $newDarkroomFlg, $newLatestResults, $userID));
